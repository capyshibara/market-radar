...
public interface InterpretedClaimRepository extends JpaRepository<InterpretedClaim, Long> {
    ...
    /** Legacy maintenance seam retained for explicit operator tooling only. Startup no longer
     *  calls it: new IMPLICATION claims are intentional and feed the reviewed-analysis lane. */
    @org.springframework.data.jpa.repository.Modifying
    @Query("update InterpretedClaim c set c.superseded = true " +
